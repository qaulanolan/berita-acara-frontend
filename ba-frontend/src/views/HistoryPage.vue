...
onMounted(async () => {
  try {
    const response = await api.getMe('http://localhost:8080/berita-acara/history');
...
async function loadFile(item) {
  ...
  try {
    const response = await api.get(`http://localhost:8080/berita-acara/history/${item.id}/file`, {
      responseType: 'blob'
    });
